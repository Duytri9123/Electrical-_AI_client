  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-800 font-sans">
      ...
      <main className="max-w-7xl mx-auto px-6 py-12">
        <h1 className="text-3xl font-bold text-slate-800 mb-6">{data?.title || "Privacy Policy"}</h1>
        <div className="bg-white rounded-xl border border-slate-200 p-6 text-sm text-slate-600 prose prose-slate max-w-none">
          {data?.content ? (
            data.content.split("\n").map((line, i) => {
              ...
              if (line.startsWith("*") && line.endsWith("*")) return <p key={i} className="text-slate-400 italic">{line.replace(/\*/g, "")}</p>;
