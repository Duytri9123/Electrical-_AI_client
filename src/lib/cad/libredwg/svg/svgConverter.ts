...
export class SvgConverter {
  ...
  private lines(
    ...
    const texts = lines.map((line, index) => {
      ...
      return `<text x="${x}" y="${y}" font-size="${fontsize}" text-anchor="${anchor}" transform="${transform}">${escapeXml(line)}</text>`
    })
  ...
  private table(entity: DwgTableEntity) {
    ...
    const svgElements = cellRects
      .map(({ x, y, width, height, cell }) => {
        ...
        const text = `<text x="${textX}" y="${textY}" font-size="${cell.textHeight}" text-anchor="middle" dominant-baseline="middle">${escapeXml(cell.text)}</text>`
  ...
  private dimension(entity: DwgDimensionEntity): BBoxAndElement | null {
    ...
    if (block) {
      return {
        bbox: block.bbox,
        element: `<use href="#${escapeXmlAttr(entity.name)}" />`
      }
    }
    return null
  }
  ...
  private insert(entity: DwgInsertEntity): BBoxAndElement | null {
    ...
    if (block) {
      ...
      return {
        bbox: newBBox,
        element: `<use href="#${escapeXmlAttr(entity.name)}" transform="${transform}" />`
      }
    }
    return null
  }
  ...
  private block(
        ...
        if (boundsAndElement) {
          ...
          const fill =
            entity.type == 'TEXT' || entity.type == 'MTEXT'
              ...
              : 'none'
          if (color.isByBlock) {
            acc.elements.push(`<g id="${escapeXmlAttr(entity.handle)}">${element}</g>`)
          } else {
            acc.elements.push(
              `<g id="${escapeXmlAttr(entity.handle)}" stroke="${color.cssColor}" fill="${fill}">${element}</g>`
            )
          }
        }
        return acc
    ...
    if (bbox.valid) {
      return {
        bbox,
        element: `<g id="${escapeXmlAttr(block.name)}">${elements.join('\n')}</g>`
      }
    }
    return null
  }
